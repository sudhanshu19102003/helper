...
class application(Frame):
    ...
    def createWidget(self):
        ...
        self.canvas.bind('<Motion>',self.activate_paint)
    ...
    def paint(self, event):
        global lastx, lasty
        x, y = event.x, event.y
        self.canvas.create_line((lastx, lasty, x, y), width=12, fill='white', capstyle=ROUND, smooth=TRUE, splinesteps=36)
        lastx, lasty = x, y
    ...
    def predictDigit(self):
        ...
        data = np.array(img)
        data = data/255 # for range b/w 0-1
